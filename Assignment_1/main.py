...
imglow = input().rstrip()
size_low = img.imread('test_cases/' + imglow + '0.png').shape # shape of the low resolution images to use as a parameter for inicializing the array of these images
low_imgs_arr = np.zeros(np.append(4, size_low)) # inicialing the array that containes the low resolution images
for i in range(4):
     low_imgs_arr[i] = img.imread('test_cases/' + imglow + str(i) + '.png') # filling in the array with the input images

imghigh = input().rstrip()
imghigh = img.imread('test_cases/' + imghigh) # the high resolution input image
